...
const cooldownDelay = parseInt(formFields['cooldownDelay']) || 300;

const RPC_ENDPOINT = 'https://jsonrpc-bfb-1.anvil.europe-west.initia.xyz';
const provider = new ethers.providers.JsonRpcProvider(RPC_ENDPOINT);

const defendLocationId = parseInt(formFields['defendLocationId']);
const units = [
    'Kabango',
    'Rootguard',
    'Gunmak',
...
const locationSizes = [
    ...
    { location: 23, size: 20 },
    { location: 24, size: 16 },
    { location: 1001, size: 16 },
    { location: 1002, size: 16 },
    { location: 1003, size: 16 },
    { location: 1004, size: 16 },
    { location: 1005, size: 16 },
    { location: 1006, size: 16 }
];
...
const SCHEMA = "ai_0xf5bc2d28d40cd3cfa98e32f7c709b784b743336a_";
...
async function getLocations() {
    ...
    //for (let i = 0; i <= 7; i++) {
    for (let locationSize of locationSizes) {
        let TABLE_ID = '0x746200000000000000000000000000004c6f636174696f6e7300000000000000';

        let KEY_TUPLE = [ethers.utils.hexZeroPad(locationSize.location, 32)];
        let TABLE_SCHEMA = {
            schema: {
                id: "uint32",
        ...
        let recordDecoded = await YeomenAI.decodeMudRecord(TABLE_SCHEMA, KEY_TUPLE, record);
        console.log('location', locationSize, recordDecoded)
...
async function getBattleById(battleId) {
    ...
    let TABLE_ID = '0x74620000000000000000000000000000426174746c6573000000000000000000';

    let KEY_TUPLE = [battleId];
    let TABLE_SCHEMA = {
        schema: {
            id: "bytes32",
            warId: "bytes32",
...
async function getBattleByDefendingArmy(defendingArmy) {
        ...
        ws.onmessage = (msg) => {
            try {
                ...
                if (data.type === 'connected') {
                    ...
                    return; // Skip resolving until the real data arrives
                }

                // Assuming this is where your battles data comes in
                console.log("Received data:", data);
                const battles = data.updates.filter(update =>
                    update.table === "Battles" && update.new
                );

                const battle = battles.find(update =>
                    update.new.defendingArmy.toLowerCase() === defendingArmy.toLowerCase()
                );
                resolve(battle?.new || null);

                // Optionally close the connection after receiving the data
                ws.close();
            } catch (error) {
    ...
    // defendingArmy = defendingArmy.replace(/(0x|\\x)/g, '\\\\x');
    // const queryData = await YeomenAI.getQueryData(`
    //         query GetQuery {
    //           ${SCHEMA}battles(where: {defending_army: {_eq: "${defendingArmy}"}}) {
    //             id                    
    //             war_id,
    //             winner,
    //             init_time,
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.onmessage = (msg) => {
            try {
                ...
                // Assuming this is where your battles data comes in
                console.log("Received data:", data);
                const units = data.updates.filter(update =>
                    update.table === "Units" && update.new && update.new.owner === owner
                );

                resolve(units.map((update) => update.new) || []);
...
(async function () {
    let stats = {
        ...
        battles_lost: 0,
        lootboxes: 0,
        rewards: { wood: 0, stone: 0, iron: 0, sugar: 0 }
    };
    ...
    while (true) {
        try {
            ...
            YeomenAI.statusMessage("Auto Defend Worker started", YeomenAI.MESSAGE_TYPES.INFO);
            ...
            let warPermitLimit = 10000;
            let warPermitBalance = parseInt(player.warPermitBalance) / Math.pow(10, 18);
            if (warPermitBalance < warPermitLimit) {
                try {
                    YeomenAI.statusMessage("Deposit warPermit", YeomenAI.MESSAGE_TYPES.INFO);
                    await YeomenAI.sendTransaction('renewWarPermit', []);
                    YeomenAI.statusMessage("Deposit warPermit Success", YeomenAI.MESSAGE_TYPES.SUCCESS);
                } catch (err) {
                    YeomenAI.statusMessage(`Deposit warPermit Failed ${err.message}`, YeomenAI.MESSAGE_TYPES.ERROR);
                }
            }
            ...
            //console.log(unitConstants)
            console.log('spawnUnits', spawnUnits)
            console.log('spawnUnitsModifiers', spawnUnitsModifiers)

            let defendLocation = locations.find((location) => location.isTeamRight == player.isTeamRight && location.id == defendLocationId);

            if (!defendLocation) {
                YeomenAI.statusMessage("Location is not available to defend, Waiting 5 minutes before next check", YeomenAI.MESSAGE_TYPES.INFO);
                await YeomenAI.delay(5 * 60);//process after x minutes
                continue;
            }

            let defendLocationSize = locationSizes.find((locationSize) => locationSize.location == defendLocationId);
            if (defendLocationSize && defendLocationSize.size !== spawnUnits.length) {
                YeomenAI.statusMessage(
                    `The required units count for location is ${defendLocationSize.size}, but spawnUnits count is ${spawnUnits.length}.`,
            ...
            let battle;
            let ongoingBattles = defendLocation.ongoingBattles;
            for (let ongoingBattleId of ongoingBattles) {
                let ongoingBattle = await getBattleById(ongoingBattleId);
                if (ongoingBattle && ongoingBattle.battleState == 0) {
            ...
            if (!battle) {
                YeomenAI.statusMessage("No Battle exists to defend, Waiting 5 minutes before next check", YeomenAI.MESSAGE_TYPES.INFO);
                await YeomenAI.delay(5 * 60);//process after x minutes
                continue;
            }
            ...
            console.log('join battle', battle)

            //await waitForMudConfigSync();


            if (army) {
                let battleExists = await getBattleByDefendingArmy(army.armyId);
                console.log('battleExists', battleExists)
                if (battleExists && battleExists.battleState != 3) {
            ...
            try {
                ...
                await YeomenAI.sendTransaction('createArmy', [[]]);
            ...
            //Check if all requiredUnits are there to attack
            await YeomenAI.delay(10);
            player = await getPlayer(playerAddress);
            for (let [unitTypeIndex, requiredUnitTypeCount] of Object.entries(requiredUnits)) {
                let playerUnitTypeCount = player.units[unitTypeIndex] || 0;
                if (playerUnitTypeCount < requiredUnitTypeCount) {
                    YeomenAI.statusMessage(
                        `Unit type ${units[unitTypeIndex]}(${unitTypeIndex}) required is ${requiredUnitTypeCount}, but only ${playerUnitTypeCount} is available.`,
                        YeomenAI.MESSAGE_TYPES.INFO
                    );
                    return YeomenAI.exit(1);
                }
            ...
            let locationId = defendLocation.id;
            try {
                YeomenAI.statusMessage(`Defend Location ${locationId}, battle ${BigInt(battleId).toString()}`, YeomenAI.MESSAGE_TYPES.INFO);
                await YeomenAI.sendTransaction('defendWithArmy', [locationId, battleId]);
                await YeomenAI.delay(2);
                YeomenAI.statusMessage(`Defend Location ${locationId}, battle ${BigInt(battleId).toString()} success`, YeomenAI.MESSAGE_TYPES.SUCCESS);
            } catch (err) {
                console.error(`Failed to defend location ${locationId}:`, err);
                YeomenAI.statusMessage(`Defend Location ${locationId}, battle ${BigInt(battleId).toString()} failed ${err.message}`, YeomenAI.MESSAGE_TYPES.FAILED);
            }
            ...
            // battle = await getBattleByDefendingArmy(armyId);
            ...
            try {
                ...
                for (let i = 0; i < spawnUnitsModifiers.length; i++) {
                    try {
                        ...
                        let battleUnitByOwner = battleUnitsByOwner.find((battleUnitByOwner) => battleUnitByOwner.unitType == unitType && battleUnitByOwner.x == x && battleUnitByOwner.y == y)
                        let unitId = battleUnitByOwner.id;
            ...
            const startTime = parseInt(battle.initTime) + 5 * 60; // Battle starts 10 minutes after init_time
            ...
            YeomenAI.statusMessage(
                `The battle will begin once ready. Battle may start and complete if opponent triggers.`,
                YeomenAI.MESSAGE_TYPES.INFO
            );
            ...
            console.log('battleRewardsToClaim', battleRewardsToClaim)
            YeomenAI.statusMessage(`Battle Rewards Claimed - Lootboxes: ${battleRewardsToClaim[0]}, Wood: ${battleRewardsToClaim[1][0]}, Stone: ${battleRewardsToClaim[1][1]}, Iron: ${battleRewardsToClaim[1][2]}, Sugar: ${battleRewardsToClaim[1][3]}`);
            stats.lootboxes += Number(battleRewardsToClaim[0]);            
            stats.rewards.wood += Number(battleRewardsToClaim[1][0]);
            stats.rewards.stone += Number(battleRewardsToClaim[1][1]);
            stats.rewards.iron += Number(battleRewardsToClaim[1][2]);
            stats.rewards.sugar += Number(battleRewardsToClaim[1][3]);
            await displayStats();
            ...
            YeomenAI.statusMessage(`Waiting ${cooldownDelay} seconds before next round`, YeomenAI.MESSAGE_TYPES.INFO);
            await YeomenAI.delay(cooldownDelay);//process after x minutes

        } catch (err) {
            console.log(err)
            YeomenAI.statusMessage(
                `Error: ${err.message || err}, Waiting ${cooldownDelay} seconds before next check`,
