    ...
    try {
        ...
        async function start() {
            ...
            while (playerCharacter.isAlive) {
                ...
                let opponentCharacters = characters.filter((character) => character.owner.toLowerCase() != playerAddress.toLowerCase() && character.isAlive && character.characterType == 1);
                ...
                if (
                    nearestOpponentCharacter.x >= playerCharacter.x - radius &&
                    nearestOpponentCharacter.x <= playerCharacter.x + radius &&
                    nearestOpponentCharacter.y >= playerCharacter.y - radius &&
                    nearestOpponentCharacter.y <= playerCharacter.y + radius
                ) {
                    console.log("Opponent is within the bounds!", nearestOpponentCharacter);
                    ...
                    // Determine direction to move based on alignment
                    let deltaDirections = [
                        { dx: 0, dy: -1 },   // Up
                        { dx: 1, dy: 0 },   // Right
                        { dx: 0, dy: 1 },   // Down                       
                    ...
                    // Set primary direction based on alignment with opponent
                    if (nearestOpponentCharacter.y === playerCharacter.y) {
                        // Opponent is aligned on the y-axis, prioritize x-direction movement
                        deltaDirections = [deltaDirections[0], deltaDirections[2], deltaDirections[3], deltaDirections[1]];
                    } else if (nearestOpponentCharacter.x === playerCharacter.x) {
                        // Opponent is aligned on the x-axis, prioritize y-direction movement
                        direcdeltaDirectionstions = [deltaDirections[3], deltaDirections[1], deltaDirections[0], deltaDirections[2]];
                    }

                    // Try each direction in order of priority until a free tile is found
                    for (const { dx, dy } of deltaDirections) {
                        const testX = playerCharacter.x + dx;
                        const testY = playerCharacter.y + dy;
                        if (isTileFree(testX, testY)) {
                            newPosition = { x: testX, y: testY };
                            console.log(`Moving to (${newPosition.x}, ${newPosition.y}) to avoid opponent.`);
                            break;
                        }
                    }
                    ...
                    // If no free tile is found, stay in place
                    console.log("Final player position:", newPosition.x, newPosition.y);


                    const heading = getHeading({ x: playerCharacter.x, y: playerCharacter.y }, { x: newPosition.x, y: newPosition.y });
                    const directionIndex = directions.indexOf(heading);
                    console.log(`Moving ${heading} ${directionIndex} to maximize distance from opponent.`);
                    // Move in the direction that maximizes distance
                    await move(playerEntity, directionIndex, characterType);



                } else {
                    console.log("Opponent is out of bounds.", nearestOpponentCharacter);
                }
